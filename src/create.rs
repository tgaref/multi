    ...
    P: AsRef<Path> + Display
{
	...
	.context(SaveFileErr { filename: PathBuf::from(TEST_PAPERS_JSON)})?;
	...
	.context(SaveFileErr { filename: PathBuf::from(CORRECT_ANSWERS_JSON)})?;
    ...
    P: AsRef<Path> + Display
{
    ...
    let mut wrt = Writer::from_path(filename).context(CsvWriterErr { filename: filename.to_path_buf() })?;
	...
	wrt.write_record(&ans).context(CsvWriterErr { filename: filename.to_path_buf() })?;
    }
    wrt.flush().context(SaveFileErr { filename: filename.to_path_buf() })?;
